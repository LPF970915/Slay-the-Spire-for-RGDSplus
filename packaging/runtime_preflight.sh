...
runtime_preflight() (
    ...
    for tool in python3 timeout mount umount mktemp bash unzip zip sha256sum; do
    ...
    if [ -n "$java_home" ]; then
        ...
        for candidate in "$APP_DIR/runtime/java" \
    ...
    case "$mode" in
        system-wayland|nested-wayland|upstream-weston)
            for candidate in "$CONTROLFOLDER/libs/$weston_name" \
