...
def main():
    ...
    def state():
        ...
        for name in ("state.xml", "dual-state.xml"):
            try:
                root = ET.fromstring(read(APP + "/logs/" + name))
...
""", 240))
            launch = (ROOT / "packaging/launch.sh").read_text()
            launch = launch.replace('    "-javaagent:$APP_DIR/rgds-input-agent.jar"',
                                    '    "-javaagent:$APP_DIR/rgds-input-agent.jar"\n'
                                    '    "-javaagent:$APP_DIR/rgds-dual-r3.jar"')
            launch = launch.replace('$APP_DIR/librgds-sdl.so:$APP_DIR/libwrap.so',
...
"""))
            ...
            before = state()
            touch_before = json.loads(read(APP + "/logs/touch-state.json"))
            print(remote(guard + """
import fcntl, os, struct, time
status=json.loads((app/'logs/touch-state.json').read_text())
...
"""))
            time.sleep(2)
            after = state()
            touch_after = json.loads(read(APP + "/logs/touch-state.json"))
...
"""))
            ...
            remote(guard)
            request, capture = APP+"/logs/capture.request", APP+"/logs/capture.pam"
            ...
            (output/(args.name+"-state.json")).write_text(json.dumps(state(),ensure_ascii=False,indent=2))
            sftp.remove(request)
            sftp.remove(capture)
